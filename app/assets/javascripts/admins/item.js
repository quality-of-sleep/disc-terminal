$(document).on("turbolinks:load", function() {
  // 全ての子孫要素から取得する.find()
  $($(".item-page").find("select, input[type=text], input[type=number]"))
    .addClass("form-control")
    .css({ display: "inline" });
  $($(".item-page").find("select")).addClass("btn btn-default dropdown-toggle");
  $($(".review-page").find("select, input[type=text], input[type=number]"))
    .addClass("form-control")
    .css({ display: "inline" });
  $($(".review-page").find("select")).addClass(
  ...
  var disc_number = function() {
    ...
    $(".disc-number").each(function() {
      $(this)
        .closest(".disc")
        .addClass("disc-" + i); // var song_numberで用いる
      $(this).val(i); // disc-number(params)を変更
      $(this)
        .prev()
        .text("Disc-" + i); // label名を変更
      i++;
    });
  };
  disc_number(); // 実行
  ...
  var song_number = function() {
    ...
    $(".disc").each(function() {
      // disc毎にループを掛ける
      k = 1;
      $(`.disc-${j} .song-number`).each(function() {
        // 変数展開にはバッククォートを用いる
        $(this).val(k);
        k++;
      });
  ...
  $(document).on("click", ".nested-fields a", function() {
    ...
    var add_form_control = function() {
      $(".disc input[type=text]")
        .addClass("form-control")
        .css({ width: "88.5%", display: "inline" });
    };
